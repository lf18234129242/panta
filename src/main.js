...
import {Button, Field, Cell, CellGroup, Popup, Picker, RadioGroup, Radio, Toast, Dialog, PasswordInput, Icon, Tab, Tabs, Uploader, NavBar, Rate, Loading, PullRefresh, Checkbox} from 'vant'
...
Vue.use(Button).use(Field).use(Cell).use(CellGroup).use(Popup).use(Picker).use(RadioGroup).use(Radio).use(Toast).use(Dialog).use(PasswordInput).use(Icon).use(Tab).use(Tabs).use(Uploader).use(NavBar).use(Rate).use(Loading).use(PullRefresh).use(Checkbox)
